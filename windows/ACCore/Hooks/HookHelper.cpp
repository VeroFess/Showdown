#ifndef _HOOKHELPER_CPP_  // why here?
#define _HOOKHELPER_CPP_
...
BOOLEAN NtApiHook(PVOID functionToHook, PVOID targetFunction) {
  ...
  if (NT_SUCCESS(NtProtectVirtualMemory(NtCurrentProcess(),
                                        &functionStartAddress, &sizeToModify,
    ...
    return NT_SUCCESS(
        NtProtectVirtualMemory(NtCurrentProcess(), &functionStartAddress,
                               &sizeToModify, PAGE_EXECUTE_READ, &oldProtect));
  } else {
    return false;
  }
}
