...
constexpr ULONG PAGE_EXECUTE_FLAGS =
...
NTSTATUS NTAPI HookedNtAllocateVirtualMemory(
    ...
    if (Protect & PAGE_EXECUTE_FLAGS) {
        ...
        Log(MSG_TRACE_V,
            "[MCPAC] [Thread] Begin Stack trace for NtAllocateVirtualMemory, try "
            "to allocate 0x%016X byte executeable memory.\n",
            *RegionSize);

        while (StackWalk64(IMAGE_FILE_MACHINE_AMD64, GetCurrentProcess(), GetCurrentThread(), &frame, &context, nullptr, SymFunctionTableAccess64, SymGetModuleBase64, nullptr)) {
            ...
            if (NT_SUCCESS(NtQueryVirtualMemory(
                    NtCurrentProcess(),
                    reinterpret_cast<PVOID64>(frame.AddrPC.Offset),
                    MemoryMappedFilenameInformation,
                    memoryMappedFilenameInformationBuffer, 1024, &returnLength))) {
                Log(MSG_TRACE_V,
                    "    The return address <0x%016X> is in module %.*ls.\n",
                    context.Rip, memoryMappedFilenameInformationBuffer->Length,
...
NTSTATUS NTAPI HookedNtProtectVirtualMemory(_In_ HANDLE ProcessHandle, _Inout_ PVOID *BaseAddress, _Inout_ PSIZE_T RegionSize, _In_ ULONG NewProtect, _Out_ PULONG OldProtect) {
    if (NewProtect & PAGE_EXECUTE_FLAGS) {
        ...
        Log(MSG_TRACE_V,
            "[MCPAC] [Thread] Begin Stack trace for NtProtectVirtualMemory, try to "
            "modify 0x%016X(0x%016X byte) memory to executeable\n",
