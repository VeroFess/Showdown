...
BOOL QuerySystemPool() {
  ...
  if (!NT_SUCCESS(NtAllocateVirtualMemory(
          NtCurrentProcess(), &poolTagInformationBuffer, 0, &bufferSize,
  ...
  for (ULONG i = 0; i < reinterpret_cast<PSYSTEM_POOLTAG_INFORMATION>(
                            poolTagInformationBuffer)
                            ->Count;
       i++) {
    if (!VerifyPool(&reinterpret_cast<PSYSTEM_POOLTAG_INFORMATION>(
                         poolTagInformationBuffer)
                         ->TagInfo[i])) {
