...
INT HandleQueryThread(PSYSTEM_HANDLE_TABLE_ENTRY_INFO HandleEntry) {
  ...
  do {
    ...
    if (NT_SUCCESS(NtQueryObject(duplicateHandle, ObjectTypeInformation,
                                 objectTypeInformationBuffer,
                                 static_cast<ULONG>(bufferSize), nullptr))) {
    ...
    if (targetProcessInformation.UniqueProcessId !=
      ...
      if (!_wcsnicmp(reinterpret_cast<POBJECT_TYPE_INFORMATION>(
                         objectTypeInformationBuffer)
                         ->TypeName.Buffer,
                     L"Process",
                     reinterpret_cast<POBJECT_TYPE_INFORMATION>(
        ...
        if (NT_SUCCESS(NtQueryInformationThread(
          ...
          if (StartAddress == LoadLibraryA || StartAddress == LoadLibraryW ||
            ...
            Log(MSG_FATAL,
                ...
                StartAddress);
            NtTerminateProcess(NtCurrentProcess(),
                               UNEXPECTED_THREAD_START_ADDRESS);
          ...
          if (NT_SUCCESS(NtQueryVirtualMemory(
            ...
            if (!(memoryBasicInformation.AllocationProtect !=
              ...
              Log(MSG_FATAL,
                  ...
                  HandleEntry->HandleValue);
              NtTerminateProcess(NtCurrentProcess(),
                                 MODIFIED_THREAD_START_PAGE_ATTRIBUTE);
            ...
            if (memoryBasicInformation.Type != SEC_IMAGE) {
              Log(MSG_FATAL,
                  ...
                  HandleEntry->HandleValue);
              NtTerminateProcess(NtCurrentProcess(),
                                 STARTING_PAGE_ATTRIBUTE_IS_NOT_IMAGE);
            ...
            if (NT_SUCCESS(status = NtQueryVirtualMemory(
              ...
              if (!VerifyLibrary(memoryMappedFilenameInformationBuffer)) {
                NtTerminateProcess(NtCurrentProcess(),
                                   UNABLE_TO_VERIFY_LOADED_LIBRARY);
              ...
              Log(MSG_FATAL,
                  ...
                  "queried.");
              NtTerminateProcess(NtCurrentProcess(),
                                 UNABLE_TO_VERIFY_STARTING_PAGE_INFORMATION);
  ...
  if (objectTypeInformationBuffer) {
    SIZE_T freeSize = 0;
    NtFreeVirtualMemory(NtCurrentProcess(), &objectTypeInformationBuffer,
                        &freeSize, MEM_RELEASE);
  }

  if (objectNameInformationBuffer) {
    SIZE_T freeSize = 0;
    NtFreeVirtualMemory(NtCurrentProcess(), &objectNameInformationBuffer,
                        &freeSize, MEM_RELEASE);
  }

  if (processNameInformationBuffer) {
    SIZE_T freeSize = 0;
    NtFreeVirtualMemory(NtCurrentProcess(), &processNameInformationBuffer,
                        &freeSize, MEM_RELEASE);
  }

  if (openedProcessNameInformationBuffer) {
    SIZE_T freeSize = 0;
    NtFreeVirtualMemory(NtCurrentProcess(), &openedProcessNameInformationBuffer,
                        &freeSize, MEM_RELEASE);
  }
...
BOOLEAN VerifyProcessHandles() {
  ...
  while (status == STATUS_INFO_LENGTH_MISMATCH) {
    SIZE_T freeSize = 0;
    if (!NT_SUCCESS(NtFreeVirtualMemory(NtCurrentProcess(),
  ...
  for (ULONG i = 0; i < systemHandleInformation->NumberOfHandles; i++) {
    HANDLE handle;
    if (NT_SUCCESS(NtCreateThreadEx(&handle, THREAD_ALL_ACCESS, nullptr,
                                    NtCurrentProcess(), HandleQueryThread,
                                    &systemHandleInformation->Handles[i], false,
  ...
  if (systemHandleInformationBuffer) {
    SIZE_T freeSize = 0;
    NtFreeVirtualMemory(NtCurrentProcess(), &systemHandleInformationBuffer,
                        &freeSize, MEM_RELEASE);
  }
