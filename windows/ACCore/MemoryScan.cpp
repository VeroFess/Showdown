...
BOOL QueryVirtualMemory() {
	...
	if (!NT_SUCCESS(NtAllocateVirtualMemory(NtCurrentProcess(), &memoryMappedFilenameInformationBuffer, 0, &bufferSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE))) {
	...
	for (ULONG64 StepAddress = 0; StepAddress < 0x00007FFFFFFFFFFF; StepAddress++) {
		if (NT_SUCCESS(NtQueryVirtualMemory(NtCurrentProcess(), (PVOID) StepAddress, MemoryBasicInformation, &memoryBasicInformation, sizeof(MEMORY_BASIC_INFORMATION), &returnedLength))) {
			if (memoryBasicInformation.Type == SEC_IMAGE) {
				ZeroMemory(memoryMappedFilenameInformationBuffer, bufferSize);
				if (NT_SUCCESS(NtQueryVirtualMemory(NtCurrentProcess(), (PVOID) StepAddress, MemoryMappedFilenameInformation, memoryMappedFilenameInformationBuffer, bufferSize, &returnedLength))) {
					if (!VerifyLibrary(reinterpret_cast<PUNICODE_STRING>(memoryMappedFilenameInformationBuffer))) {
						NtTerminateProcess(NtCurrentProcess(), UNABLE_TO_VERIFY_LOADED_LIBRARY);
						__fastfail(UNABLE_TO_VERIFY_LOADED_LIBRARY);
					}
				} else {
					Log(MSG_FATAL, "[MCPAC] [Fatal] The memory attribute of this area is an executable program, but related information cannot be queried.");
