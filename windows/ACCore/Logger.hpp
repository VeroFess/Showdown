...
static LPVOID LoggerMalloc(SIZE_T byteToAlloc) {
    ...
    if (NT_SUCCESS(NtAllocateVirtualMemory(NtCurrentProcess(), &pBase, nullptr, &allocSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE))) {
...
static void LoggerInitize(UNICODE_STRING file) {
    ...
    InitializeObjectAttributes(&OAF, &USF, OBJ_CASE_INSENSITIVE, nullptr, nullptr);
...
static void Log(INT level, PCCH format, ...) {
    ...
    if (length != -1) {
        ...
        if (StdOutput != INVALID_HANDLE_VALUE && StdOutput) {
            NtWriteFile(StdOutput, nullptr, nullptr, nullptr, &FIOSB, reinterpret_cast<PVOID>(stringBuffer), length, &FO, nullptr);
        }

        NtWriteFile(LogFileHandle, nullptr, nullptr, nullptr, &FIOSB, reinterpret_cast<PVOID>(stringBuffer), length, &FO, nullptr);
    }
